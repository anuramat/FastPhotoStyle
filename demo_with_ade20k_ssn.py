...
parser.add_argument(
    "--arch_encoder", default="resnet50_dilated8", help="architecture of net_encoder"
)
parser.add_argument(
    "--arch_decoder", default="ppm_bilinear_deepsup", help="architecture of net_decoder"
)
parser.add_argument(
    "--fc_dim",
    default=2048,
...
parser.add_argument(
    "--batch_size", default=1, type=int, help="batchsize. current only supports 1"
)
parser.add_argument(
    "--imgSize",
...
parser.add_argument(
    "--imgMaxSize", default=1000, type=int, help="maximum input image size of long edge"
)
parser.add_argument(
    "--padding_constant",
    default=8,
...
args.weights_encoder = os.path.join(
    SEG_NET_PATH, args.model_path, "encoder" + args.suffix
)
args.weights_decoder = os.path.join(
    SEG_NET_PATH, args.model_path, "decoder" + args.suffix
)
...
net_encoder = builder.build_encoder(
    arch=args.arch_encoder, fc_dim=args.fc_dim, weights=args.weights_encoder
)
